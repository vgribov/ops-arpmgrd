...
class arpManagerFunctionalityTests( HalonTest ):
  ...
  def arp_manager_neighbor_fail(self):
    ...
    info("Address deleted. Waiting for 45 seconds before checking Neighbor entries\n");
    ...
    assert words.index(max(words)) == max_index, "MAC address still present"
    state = words[state_index]
  ...
  def arp_manager_dp_hit(self):
    info("\n\n=====================================================================\n")
    info("*** Test to verify arpmgr checks dp hit bit.                      ***\n")
    info("*** If dp hit is not set, let kernel stale out entry              ***\n")
    info("*** If dp hit is empty or true keep entry reachable by probing    ***\n")
    info("=====================================================================\n")
    # configuring Halon, in the future it would be through
    # proper Halon commands
    s1 = self.net.switches[ 0 ]
    ...
    info(output + "\n\n")
    info("\nVerifying states of host. 2000::2 should be stale. The rest of entries should be reachable\n")
    words = host1v4.split()
    assert words.index(max(words)) == max_index, "Unknown number of columns"
    state = words[state_index]
    assert state == 'reachable', "State incorrect, should be reachable"
  ...
  def arp_manager_neighbor_delete(self):
    info("\n\n=====================================================================\n")
    info("*** Test to verify arpmgr deletes row when kernel row             ***\n")
    info("*** gets deleted                                                  ***\n")
    ...
    assert host2v4 == None, "Host 2 IP 192.168.2.2 still present in Neighbor table"

    info("Neighbor 192.168.2.2 deleted\n");

    # configure 192.168.2.1 address back on switch interface 2, ping from host 2
